...
const moviesEl = document.getElementsByClassName('movies')[0];
const catButtonsEl = document.getElementById('sidebar');
const searchInput = document.getElementById('search');
const cartItemsEl = document.getElementById('cart-items');


function createMovieDiv(movieObject) {
    let divEl = document.createElement('div');
    divEl.className = 'movie';
    divEl.innerHTML = `
     <img src="img/${movieObject.poster}" class="movie__poster">
     <div class="movie__info">
         <h2 class="movie__title">${movieObject.title}</h2>
         <h3 class="movie__genre">${movieObject.genres}</h3>
         <button class="movie__btn" type="button" data-id="${movieObject.id}">Add to watchlist</button>
     </div>
    `;
    return divEl;
...
function filterMoviesByCat(category) {
...
if (catButtonsEl) {
    catButtonsEl.addEventListener('click', function (event) {
        if (event.target.className.includes('genre') || event.target.className.includes('genre__item')) {
            let selectedCat = event.target.innerText;
            let selectedMovies;
            if (selectedCat !== 'All') {
                selectedMovies = filterMoviesByCat(selectedCat);
            } else {
...
function appendSelectedMovies(movies) {
    movies.forEach((el) => {
        let movieDiv = createMovieDiv(el);
        moviesEl.appendChild(movieDiv);
    });
...
function searchMovie(searchText) {
    let resultArr = movies.filter((el) => {
        return el.title.toLowerCase().includes(searchText) || el.title.toUpperCase().includes(searchText);
    })
    return resultArr;
...
if (searchInput) {
    searchInput.addEventListener('keyup', function () {
        let searchedMovies = searchMovie(searchInput.value);
        moviesEl.innerHTML = '';
...
if (moviesEl) {
    moviesEl.addEventListener('click', function (event) {
        if (event.target.className == 'movie__btn') {
            ...
            addMovieToCart(idOfCurrentMovie);
...
function cartItemsFromLocalStorage() {
    let result = JSON.parse(localStorage.getItem('watchlist'));
    return result ? result : [];
}

function createCartItemMovie(el) {
    let divEl = document.createElement('div');
    divEl.className = 'movie';
    divEl.innerHTML = `
    ...
    <div class="movie__info">
        ...
        <button class="remove__btn" type="button" data-id="${el.id}">Remove from watchlist</button>
    </div>
...
function showCartItems() {
    ...
    let basketArr = cartItemsFromLocalStorage();
    basketArr.forEach((el) => {
        let movie = movies.filter((movie) => {
            return movie.id == el;
        });
        let element = createCartItemMovie(movie[0]);
...
if (cartItemsEl) {
    cartItemsEl.addEventListener('click', function (event) {
        if (event.target.className == 'remove__btn') {
            let elId = event.target.getAttribute('data-id');
            let basketArr = cartItemsFromLocalStorage();
            basketArr.splice(basketArr.indexOf(+elId), 1);
            localStorage.setItem('watchlist', JSON.stringify(basketArr));
